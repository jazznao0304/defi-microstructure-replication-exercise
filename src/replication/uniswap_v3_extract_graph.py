...
POOL_HOUR_DIR = RAW_DIR / "pool_hour_data"
SWAPS_DIR     = RAW_DIR / "swaps"

for d in (META_DIR, POOL_HOUR_DIR, SWAPS_DIR):
...
# ----------------------------
# Fetch pools, poolHourData, & swaps
# ----------------------------
...
def fetch_pool_hour_data(client: Client, pool_id: str, start_ts: int, end_ts: int) -> pd.DataFrame:
    """
    Fetch v3 poolHourDatas (hourly prices/liquidity/volumes).
    ...
    query($pool: ID!, $start: Int!, $end: Int!, $skip: Int!) {
      poolHourDatas(first: 1000, skip: $skip,
    ...
    # Numeric casting (subgraph fields are BigDecimal strings in token units; sqrtPrice/liquidity are integers)
    num_cols = [
        "periodStartUnix", "liquidity", "sqrtPrice",
    ...
    # Derived prices (token1 per token0) computed from sqrtPrice (Q64.96) and token decimals
    sp = pd.to_numeric(df.get("sqrtPrice"), errors="coerce")
    if sp.notna().any():
        sp = sp.astype("float64")
    ...
    df.rename(columns={"periodStartUnix": "hourStartUnix"}, inplace=True)
...
def fetch_swaps(client: Client, pool_id: str, start_ts: int, end_ts: int) -> pd.DataFrame:
    """
    Fetch v3 swaps for a pool (timestamps and amounts).
    CHUNK is a paging window (not granularity): we iterate in ~3-day slices to stay under API limits.
    """
    rows: List[dict] = []
    q = gql("""
    query($pool: Bytes!, $start: Int!, $end: Int!, $skip: Int!) {
    ...
    # Numeric casts and dedupe (parity with v2)
    for c in ("timestamp", "amount0", "amount1", "amountUSD", "sqrtPriceX96", "tick"):
        if c in df.columns:
            df[c] = pd.to_numeric(df[c], errors="coerce")
    if "id" in df.columns:
...
def run_uniswap_v3_extract_graph(
    start_ts: int = START,
    end_ts: int = END,
    ...
    """Fetch Uniswap v3 poolHourData and swaps; save Parquet under data/raw/uniswap_v3."""
    client = _gql_client()
    ...
    ph_dir = POOL_HOUR_DIR / out_subdir if out_subdir else POOL_HOUR_DIR
    sw_dir = SWAPS_DIR / out_subdir if out_subdir else SWAPS_DIR
    ...
    for s in specs:
        ...
        for p in cand:
            ...
            if outp.exists() and outs.exists():
                ...
                ph = fetch_pool_hour_data(client, pid, start_ts, end_ts)
                if not ph.empty:
                    ph["pool_id"] = pid
                    ph["feeTier"] = tier
                    ph.to_parquet(outp, index=False)
                    print(f"Saved poolHourData -> {outp}")
                else:
                    print("No poolHourData returned for this window.")

                sw = fetch_swaps(client, pid, start_ts, end_ts)
                if not sw.empty:
